...
# A bailout signal that causes the generator to backtrack a number of turns equal to "turns".
class BailoutSignal:
...
# Fills in a random cell as a clue.
def fill_random_clue(s, width, easy=False, curr_progress=None):
    ...
    # If a cell and its neighbors are all already determined, disqualify it from being a clue.
    ...
    # Weight the potential clue positions such that the positions that are farther away from the existing clues are more likely to get chosen.
    ...
    # Set the probability distribution of potential clues (can be a blank, 0, 1, 2, 3, or 4)
    toChooseFrom = "4"
    ...
    # Filter out clues that are impossible for the given grid.
...
# Usually returns False, but occasionally returns a BailoutSignal to get the puzzle "unstuck".
def random_return():
...
def generate_puzzle_recursive(s, width, depth, depth_limit, prev_prop=0, lp_hints=None, strategies=STRATEGIES[:], verbose=True):
    ...
    if verbose: print(end=f"\r{depth}     {random.randint(1000,9999)}     {len(lpDeductions)}     {currProgress.count('.')/len(s):.3f}          ")

    for _ in range(20+int(1/(1-solvedProp))):
